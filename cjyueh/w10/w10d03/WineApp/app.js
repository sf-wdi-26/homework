var app = angular.module('wineApp', ['ngRoute']); // add ngRoute module
...
app.config(function($routeProvider, $locationProvider){
  ...
  $routeProvider
    .when('/', { //root route for index
      // template: 'Home!' //check to see route is working with template string
      templateUrl: 'templates/wines-index.html', //use template file instead of string
      controller: 'WinesIndexCtrl' //specify controller for the template
    })
  ...
  $locationProvider.html5Mode({ //so don't need /#/ in url
    enabled: true,
    requireBase: false
  });
...
app.controller('WinesIndexCtrl',function($scope, $http){
  ...
  $scope.hello = "Hello from Wines Index Ctrl"; //add a value for the controller
  // $scope.wines = WineService.query(); //will need to pass WineService into function
  $http
...
app.controller('WinesShowCtrl',function($scope, $http, $routeParams){
  ...
  $http
    .get('http://daretoexplore.herokuapp.com/wines/' + wineId)
    .then(function(res,err) {
      if (err) {
        console.log("error: " + err);
      } else {
