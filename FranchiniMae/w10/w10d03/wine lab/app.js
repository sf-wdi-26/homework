...
//dont need to pass $http in here because then you'll have a scope within a scope
...
app.controller('WinesIndexCtrl',function($scope, $http){
  ...
  $scope.hello = "Hello from wines index controller";
  $http
...
app.controller('WinesShowCtrl',function($scope, $routeParams, $http){
  ...
  //for complex ids, use parseInt
  $http
    .get('http://daretoexplore.herokuapp.com/wines/' + $routeParams.id)
    .then(function(res) {
        console.log(res.data);
        $scope.wine = res.data;
    });
...
// dont need wineService above beacuse we are making a call to the daretoexplore app
