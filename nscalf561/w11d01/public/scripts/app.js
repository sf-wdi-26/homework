...
LoginController.$inject = ["Account", '$auth', '$location']; // minification protection
function LoginController (Account, $auth, $location) {
  var vm = this;
...
LogoutController.$inject = ["Account", "$location", "$auth"]; // minification protection
function LogoutController (Account, $location, $auth) {
  $auth
    .logout()
    .then(function() {
      $location.path('/login');
